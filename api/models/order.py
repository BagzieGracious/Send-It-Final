...
class Orders:
    ...
    def get_order(self, order_id=None, user_id=None):
        ...
        if user_id is None:
            ...
            sql = "SELECT * FROM orders, users WHERE orders.user_id = users.user_id AND orders.order_id = '{}'".format(order_id)
            self.cursor.execute(sql)
            return self.cursor.fetchone()

        if order_id is None:
            sql = "SELECT * FROM orders, users WHERE orders.user_id = users.user_id AND orders.user_id = '{}'".format(user_id)
            self.cursor.execute(sql)
            return self.cursor.fetchall()

        sql = "SELECT * FROM orders, users WHERE orders.user_id = users.user_id AND orders.user_id = '{}' AND orders.order_id = '{}'".format(user_id, order_id)
        self.cursor.execute(sql)
        return self.cursor.fetchone()
