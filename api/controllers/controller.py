...
class Controller:
    ...
    def login(self):
        ...
        if user_data:
            ...
            return jsonify({"usertype": user_data['usertype'], "status": "success", "message": "you have logged in successfully","token": token.decode('UTF-8')}), 200
    ...
    def get_users(self):
        ...
        if self.user.get_all_users((self.token_value())['user_id']):
            return jsonify({"status": "success", "data": self.user.get_all_users((self.token_value())['user_id'])}), 200
        return jsonify({"status": "failure", "error": {"message": "no user found"}}), 404
